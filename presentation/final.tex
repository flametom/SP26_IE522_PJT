...
% ============================================================
% SLIDE 2: RECAP --- Problem & Motivation (from midterm)
% ============================================================
...
\end{columns}
\source{Shi et al.\ (2024) --- presented at midterm}
\end{frame}
...
\end{tikzpicture}
\end{center}
\source{Shi et al.\ (2024), Section III --- presented at midterm}
\end{frame}
...
\end{columns}
\source{Shi et al.\ (2024), Section III, Tables II--III --- presented at midterm}
\end{frame}
...
% ============================================================
\begin{frame}[t,fragile,shrink=8]{Engineering \textcircled{\scriptsize 2}: From midterm A* to batch SSSP}
\footnotesize

\textbf{Midterm baseline:} per-agent A* (Euclidean heuristic, pure Python).\\
\textbf{Insight for post-midterm:} many agents share destinations (buildings, shelters). Compute \emph{one} shortest-path tree per unique destination, reuse across agents.
{\scriptsize (SSSP = Single-Source Shortest Path tree, one source $\to$ all targets.)}

\vspace{0.4em}
\begin{columns}[T,onlytextwidth]
\column{0.49\textwidth}
\begin{block}{\scriptsize Midterm version (per-agent A*)}
\vspace{-0.3em}
...
\column{0.49\textwidth}
\begin{block}{\scriptsize Post-midterm (batch SSSP)}
\vspace{-0.3em}
\begin{semiverbatim}\tiny
...
% ============================================================
\begin{frame}[t,fragile,shrink=5]{Engineering \textcircled{\scriptsize 3}: Where A* is still used, per-step recomputation, panic noise}
\footnotesize

\begin{columns}[T,onlytextwidth]
\column{0.50\textwidth}
\begin{block}{\scriptsize A* (from midterm) is kept for cases where batch SSSP can't help}
\scriptsize
\begin{itemize}\itemsep0.1em
  \item \textbf{Shelter redirect} (agent just got impacted, destination changed mid-step)
  \item \textbf{Congestion-aware routing} (edge-weight depends on current flow; no shared tree)
  \item \textbf{Panicked agents} (noisy weights $\Rightarrow$ per-agent random)
\end{itemize}
\vspace{-0.3em}
\begin{semiverbatim}\tiny
def heuristic(u, v):   # Euclidean
    ux, uy = coords[u]; vx, vy = coords[v]
    return ((ux-vx)**2 + (uy-vy)**2) ** 0.5
\end{semiverbatim}
$\sim$2--3$\times$ faster than plain Dijkstra on spatial graphs.
\end{block}
...
% ============================================================
\begin{frame}[t,shrink=10]{Engineering \textcircled{\scriptsize 5}: Benchmark --- midterm A* vs batch SSSP}
\footnotesize
...
\begin{columns}[T,onlytextwidth]
\column{0.48\textwidth}
\begin{alertblock}{\small Total speedup (batch vs midterm A*)}
\scriptsize
\textbf{7.5$\times$} single-process (batch SSSP, scipy C)\\
\textbf{$\times$ 8 workers $=$ $\sim$60$\times$} overall\\[2pt]
$\Rightarrow$ 390 runs (8-worker parallel): \\ midterm A* $\sim$ 2.3 h $\to$ batch SSSP $\sim$ 18 min
\end{alertblock}
...
\textbf{Engineering}: 7.5$\times$ single-process speedup (scipy batch SSSP), $\times$8 parallel $\Rightarrow$ 390 runs in a few hours.\\
...
\begin{itemize}\itemsep0.25em
  ...
  \item \textbf{Algorithm / engineering improvement}:\\
        midterm per-agent A* $\to$ batch SSSP (7.5$\times$ single-process, 60$\times$ overall)
