...
\begin{tikzpicture}[>=Stealth, font=\scriptsize, scale=0.95, transform shape]
  ...
  \node[below=0.1em of ext, font=\tiny] {510 runs (separate)};
\end{tikzpicture}
\end{center}
\end{frame}
...
\column{0.48\textwidth}
\begin{center}
\includegraphics[width=\linewidth]{assets/benchmark_time_dist.pdf}
\end{center}
...
\begin{itemize}\itemsep0.15em
  \item Algorithm 1 path recomputation is\\
        $\approx$\,\keyterm{99\% of a single run}\\
        {\scriptsize (165.9s of 168s, naïve)}
  ...
  \item Flow bookkeeping, state transitions, hazard checks = other 1\%
\end{itemize}
\end{columns}
...
\begin{tikzpicture}[>=Stealth, font=\scriptsize, scale=0.88, transform shape]
  ...
  \node[draw=gray!40, dashed, rounded corners, fill=lightgrayfill,
        minimum width=11.5cm, minimum height=2.4cm] (bg) at (0, 0) {};
  \node[anchor=south, font=\tiny, text=gray, inner sep=1pt] at (bg.north) {Graph $G = (V, E)$};

  % --- Panel A: Per-agent queries (left) ---
  \node[font=\scriptsize\bfseries, text=highlightblue] at (-4.0, 0.95) {Per-agent queries};

  \node[panelbox, fill=lightbluefill, minimum width=0.55cm, minimum height=0.35cm, text width=0.45cm, align=center, inner sep=1pt] (p1) at (-5.5, 0.5) {$p_1$};
  \node[panelbox, fill=lightbluefill, minimum width=0.55cm, minimum height=0.35cm, text width=0.45cm, align=center, inner sep=1pt] (p2) at (-5.5, 0.05) {$p_2$};
  \node[font=\tiny, text=gray] at (-5.5, -0.38) {$\vdots$};
  \node[panelbox, fill=lightbluefill, minimum width=0.55cm, minimum height=0.35cm, text width=0.45cm, align=center, inner sep=1pt] (pn) at (-5.5, -0.8) {$p_N$};
  ...
  \node[panelbox, fill=lightbluefill, minimum width=3.8cm, minimum height=0.35cm, text width=3.7cm, align=center, font=\tiny, inner sep=1pt] (agents) at (3.8, -0.85) {agents: predecessor-tree lookup in $T(d_{p_i})$};
...
\begin{center}
\fcolorbox{highlightblue}{lightbluefill}{\parbox{0.92\linewidth}{\centering\scriptsize
Per-agent path loop: \textbf{165.9\,s $\to$ 0.7\,s} (algo1 only).
But shared-tree construction adds \textbf{+19.3\,s} of new overhead.\\
\textbf{Routing subsystem net: 165.9\,s $\to$ 20.0\,s $\approx$ 8.3$\times$.}
End-to-end: 168\,s $\to$ 22.5\,s $\approx$ \textbf{7.5$\times$} single-process.
}}
\end{center}
...
% ============================================================
\begin{frame}[t,shrink=15]{Gap B: Running 390 experiments, honestly}
\footnotesize
...
\textbf{We chose:} agent seeds vary across runs (\texttt{42, 142, 242, \ldots}); \keyterm{hazard seed is fixed at 1135 across the entire sweep}; sim-dynamics seed = agent + 2000.\\
\textbf{Because:} without separation, varying $P_{\max}$ also varies hazard placements --- can't tell whether $\Delta RI$ comes from people or from hazards. Trade-off: SD reflects agent-level variability \emph{conditional on} the fixed hazard scenario.
\end{block}
...
\begin{itemize}\itemsep0.3em
  ...
  \item \textbf{Ext 1:} 17 $\times$ 3 $P_{\max}$/$H_{\max}$ grid\\
        {\scriptsize (510 separate runs; paper: 3 $\times$ 3 configs)}
...
\column{0.52\textwidth}
\begin{center}
\animategraphics[poster=first,controls,loop,width=0.7\linewidth]{6}{assets/frames_psuup/sim_}{000}{060}
\end{center}
...
\begin{itemize}\itemsep0.1em
  ...
  \item Matches paper's Fig.\ 5 within 2\%p (see slides 13--14).
\end{itemize}
...
% ============================================================
\begin{frame}[t,shrink=15]{Phase 1: PSU-UP network (Fig.\ 3) \& flow snapshots (Fig.\ 4)}
\footnotesize

\begin{center}
\includegraphics[height=4.2cm]{../results/fig3_network_PSU-UP.png}
\end{center}
{\centering\scriptsize \textbf{Fig.\ 3 reproduction} --- PSU-UP walk network (969 buildings, $\sim$600 shelters)\par}

\vspace{0.3em}
\begin{center}
\includegraphics[width=0.88\linewidth]{../results/fig4_flow_PSU-UP.png}
\end{center}
{\centering\scriptsize \textbf{Fig.\ 4 reproduction} --- pedestrian flow snapshots at $t=0,30,60,90,120$ min \quad
\textcolor{statenormal}{$\bullet$ Normal} \
\textcolor{statequeuing}{$\bullet$ Queuing} \
\textcolor{stateimpacted}{$\bullet$ Impacted} \
\textcolor{statesurvival}{$\bullet$ Survival} \
\textcolor{statecasualty}{$\times$ Casualty} \
\textcolor{highlightred}{$\bigcirc$ Hazard}\par}
...
\vspace{0.2em}
\begin{itemize}\itemsep0.25em
  \item $RI$ slightly higher (MAE $\approx$ 5\%p)
...
\vspace{0.4em}
\textbf{Calibrated assumptions} (held fixed for all subsequent experiments) --- because the paper does not publish:
\vspace{0.2em}
...
\end{enumerate}
\vspace{0.2em}
{\scriptsize\textcolor{gray}{$\Rightarrow$ Calibrated, then held fixed; not independent validation.}}
...
\scriptsize
At high $P_{\max}$, average agents per building exceeds the \textbf{effective egress capacity} of building$\to$network connectors and nearby walk edges $\to$ outdoor queues $\to$ longer hazard exposure.\\[3pt]
Building node capacity in our config = 100; effective bottleneck is upstream of that. Connector-capacity sensitivity is future work.
\end{alertblock}
...
\scriptsize
Under our hazard-generation process, $RI$ approaches an empirical plateau near 86\% by $H_{\max}$=25--30 (verified up to 30).\\[4pt]
The remaining $\sim$14\% appears to be \emph{low-exposure} or peripheral origins under this scenario; mapping never-impacted nodes is future work.
\end{alertblock}
...
\begin{itemize}\itemsep0.1em
  ...
  \item no abrupt transition at 10\% resolution
...
$\Rightarrow$ Within this model, marginal panic reduction has the largest effect at \emph{low-baseline} panic.
\end{alertblock}
...
\begin{center}
\animategraphics[poster=first,controls,loop,width=0.55\linewidth]{6}{assets/frames_cmp_pmax/cmp_}{000}{060}
\end{center}
...
\begin{center}
\animategraphics[poster=first,controls,loop,width=0.55\linewidth]{6}{assets/frames_cmp_panic/cmp_}{000}{060}
\end{center}
...
\begin{center}
\animategraphics[poster=first,controls,loop,width=0.55\linewidth]{6}{assets/frames_cmp_hmax/cmp_}{000}{060}
\end{center}
...
\begin{itemize}\itemsep0.2em
  ...
  \item $RI$ trend reproduced (independent of $P_{\max}$);\\
        quantitative MAE $\approx$ 5\%p, max $\approx$ 9\%p
  \item $RS$/$RC$/$RL$ qualitative trends reproduced;\\
        high-panic $RC$ undershoots paper
...
Enabled multi-seed reproduction + 3 extensions; agent-side decision logic preserved verbatim.}}
\end{center}
...
\begin{tikzpicture}[font=\footnotesize, scale=0.95, transform shape]
  ...
  \node[draw=highlightred, thick, rounded corners, fill=lightredfill, minimum width=2.2cm, minimum height=1.0cm, align=center] at (6.5, 0)
    {\textbf{\large 0}\\ {\tiny new behavioral mechanisms}};
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{itemize}\itemsep0.25em
  ...
  \item \textbf{Demographic-specific shelter preference}: deferred (low expected effect on headline metrics)
  \item \textbf{Hazard-seed sensitivity sweep}: validate plateau / breakpoint claims under different hazard scenarios
\end{itemize}
\end{alertblock}
\end{columns}
...
The data-side directions (right) plug into the existing pipeline and can be run on the same fast infrastructure.
}}
\end{center}
\end{frame}
