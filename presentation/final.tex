...
\column{0.46\textwidth}
\vspace{0.1em}
\begin{alertblock}{\small Midterm version}
\scriptsize
Per-agent shortest-path query, one call at a time, in pure Python.\\[3pt]
...
% ============================================================
\begin{frame}[t,fragile,shrink=8]{Engineering \textcircled{\scriptsize 2}: From per-agent queries to batch shortest-path trees}
\footnotesize

\textbf{Midterm baseline:} each agent independently queries the graph for its own shortest path to destination. With 2{,}000 agents $\times$ 121 steps, that is $\sim$200K independent shortest-path queries per run.

\vspace{0.3em}
\textbf{Post-midterm insight:} many agents share destinations --- the same building, the same shelter. Compute \emph{one} shortest-path \emph{tree} per unique destination; all agents heading there reuse that tree with a constant-time lookup.

\vspace{0.4em}
\begin{columns}[T,onlytextwidth]
\column{0.49\textwidth}
\begin{block}{\scriptsize Midterm (per-agent query)}
\vspace{-0.3em}
...
\column{0.49\textwidth}
\begin{block}{\scriptsize Post-midterm (batch tree)}
\vspace{-0.3em}
\begin{semiverbatim}\tiny
...
\midrule
Per-agent query (midterm) & $O(N)$ & $O(N \cdot (V{+}E) \log V)$ \\
Batch tree (post-midterm) & $O(K)$ & $O(K \cdot (V{+}E) + V)$ \\
\bottomrule
\end{tabular}}
\end{center}
...
% ============================================================
\begin{frame}[t,shrink=10]{Engineering \textcircled{\scriptsize 5}: Benchmark --- midterm vs post-midterm}
\footnotesize
...
$\Rightarrow$ 390 runs (8-worker parallel): \\ midterm $\sim$ 2.3 h $\to$ post-midterm $\sim$ 18 min
\end{alertblock}
